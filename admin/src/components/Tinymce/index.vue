...
<script>
 ...
 `advlist anchor autolink autosave code codesample colorpicker colorpicker
  ...
  export default {
    ...
    watch: {
      value(val) {
        ...
        if (!this.hasChange && this.hasInit) {
          ...
          // arrs = val.split(',');
          // arrs.forEach((el,idx) => {
          //   str += '<img class="wscnph" src="'+el+'" data-mce-src="'+el+'">';
          // });
          // str += '</p>↵<p><audio style="display: none;" controls="controls"></audio></p>';
          this.$nextTick(() => window.tinymce.get(this.tinymceId).setContent(this.filterValue(val)))
        }
      }
    },
    mounted() {
    ...
    methods: {
      filterValue(val){
        if(val.indexOf('<img') != -1)return val;
        console.log(val)
        let arrs = [],str = '<p>';
        arrs = val.split(',');
        arrs.forEach((el,idx) => {
      ...
      initTinymce() {
        ...
        window.tinymce.init({
          ...
          init_instance_callback: editor => {
            if (_this.value) {
              editor.setContent(_this.filterValue(_this.value))
            }
            _this.hasInit = true
